...
export function formatSeatType(seatType) {
    if (typeof seatType !== 'string' || !seatType.includes('_')) {
        return 'ข้อมูลที่นั่งไม่ถูกต้อง'; // Handle invalid input gracefully
    }
    ...
    return `${classLevel} - ${seatCategory} ${airConditioning}`.trim();
}
